...
class HavenNestCrawler:
    def __init__(self):
        self.all_listings = []
        self.seen_urls = set() # 👈 核心：物理去重，确保 URL 唯一
    ...
    def ai_translate(self, text):
        ...
        try:
            # 采用意译逻辑：处理前 200 字即可
            return self.translator.translate(self.clean_illegal_chars(text)[:200])
        except:
            return text

    def crawl_craigslist(self, limit=12):
        print("🔍 正在同步 Craigslist 房源...")
        ...
        try:
            ...
            for item in items:
                ...
                self.all_listings.append({
                    ...
                    "image": "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800" # 统一占位图提高加载速度
                })
    ...
    def save(self):
        with open('listings.json', 'w', encoding='utf-8') as f:
            ...
            f.write(re.sub(r'[\u2028\u2029]', '', json_data))
        print(f"📊 任务结束：Havennest 已存入 {len(self.all_listings)} 条纯净数据。")
